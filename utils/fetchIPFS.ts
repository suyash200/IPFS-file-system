import { PinataFile } from "@/app/dashboard/page";
...
// Example usage
export async function downloadAndUseFile(file: PinataFile): Promise<void> {
  ...
  try {
    console.log()
    const fileBlob = await fetchIPFSFile(file?.ipfs_pin_hash!, gatewayToken);
